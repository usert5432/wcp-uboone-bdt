...
int main( int argc, char** argv )
{
  ...
  float lee_strength_type2 = 100; // 100 + LEE strength; a different style with LEE drawn but not in GoF or error calculation; based on an external file for LEE spectrum 
  int flag_display = 1;
  ...
  for (Int_t i=1;i!=argc;i++){
    ...
    case 'l':{
      lee_strength_type2 = atof(&argv[i][2]);
      if(lee_strength_type2>100) lee_strength=0;
      else lee_strength = lee_strength_type2;
    }break;
    case 'd':{
  ...
  if (flag_err==3){
    ...
    if( lee_strength_type2<100 && abs(lee_strength-cov_lee_strength)>1e-6 ) {
        std::cout<<"ERROR: plot_hist -l option is inconsistent with external cov matrix LEE strength!\n";
        return 1;
    }
    if( lee_strength_type2>100 && cov_lee_strength>1e-6 ) {
        std::cout<<"ERROR: type2 style, LEE strength is always to be 0!\n";
        return 1;
    }
    ...
    for (auto it = map_obsch_histos.begin(); it!= map_obsch_histos.end(); it++){
      ...
      for (int i=0;i!=h1->GetNbinsX()+1;i++){
        ...
        for(int j=0; j!=h1->GetNbinsX();j++){
            ...
            // hack: remove some bins
            if(obsch==991 || obsch==992)
            //if(obsch==1 || obsch==2 || obsch==3 || obsch==4)
            //if(obsch==1 || obsch==2 || obsch==3 || obsch==4 || obsch==12 || obsch==13 || obsch==14 || obsch==15)
            {
                if( (index>=0 && index<=5) || (jndex>=0 && jndex<=5) ) continue;
            }
            /* if(obsch==5 || obsch==7 || obsch==9) */
            /* { */
            /*     if( (index>=10 && index<=18) || (jndex>=10 && jndex<=18) ) continue; */
            /* } */
            /* if( obsch==2 && ( (index==0 || index>=7 && index<=9) || (jndex==0 || jndex>=7 && jndex<=9)) ) continue; */
            /* if( obsch==3 && ( (index==0 || index>=7 && index<=9) || (jndex==0 || jndex>=7 && jndex<=9)) ) continue; */
            /* if( obsch==4 && ( (index==0 || index>=8 && index<=9) || (jndex==0 || jndex>=8 && jndex<=9)) ) continue; */
            /* if( obsch==6 && ( (index==0 || index>=8 && index<=9) || (jndex==0 || jndex>=8 && jndex<=9)) ) continue; */
            /* if( obsch==7 && ( (index==0 || index==9) || (jndex==0 || jndex==9)) ) continue; */
            /* if( obsch==1 && ( (index==0 || index>=7 && index<=9) || (jndex==0 || jndex>=7 && jndex<=9)) ) continue; */
            /* if( obsch==2 && ( (index==0 || index>=8 && index<=8) || (jndex==0 || jndex>=8 && jndex<=8)) ) continue; */
            /* if( obsch==3 && ( (index>=0 && index<=1 || index>=8 && index<=8) || (jndex>=0 && jndex<=1 || jndex>=8 && jndex<=8)) ) continue; */
            /* if( obsch==4 && ( (index==0 || index>=9 && index<=9) || (jndex==0 || jndex>=9 && jndex<=9)) ) continue; */
            /* if( obsch==5 && ( (index>=5 && index<=9) || (jndex>=5 && jndex<=9)) ) continue; */
            /* if( obsch==6 && ( (index>=0 && index<=1) || (jndex>=0 && jndex<=1)) ) continue; */
            /* if( obsch==7 && ( (index==10) || (jndex==10)) ) continue; */
            /* if( obsch==9 && ( (index>=0 && index<=5) || (jndex>=0 && jndex<=5)) ) continue; */
  ...
  else // flag_breakdown == true
  {
    ...
    for(auto it = map_obsch_subhistos.begin(); it!= map_obsch_subhistos.end(); it++){
        ...
        TPad *pad1 = new TPad("pad1", "", 0.01,0.35,0.99,0.99,0,0,0);
        TPad *pad2 = new TPad("pad2", "", 0.01,0.01,0.99,0.35,0,0,0);
        ...
        legend[obschannel-1] = new TLegend(0.25, 0.59, 0.88, 0.93);
        ...
        gr[obschannel-1]->SetName("data");
        legend[obschannel-1]->SetNColumns(2);
        // numi channels
        if(obschannel>=8) legend[obschannel-1]->AddEntry((TObject*)0, Form("NuMI POT: %.3e", datapot_numi*scalePOT), "");
        ...
        TH1F* hmclee;
        if (obschannel==1 && lee_strength_type2>100) {
            TFile* leefile = new TFile(Form("nueCC_FC_leefile_LEEx%d.root", (int)lee_strength_type2-100));
            hmclee = (TH1F*)leefile->Get("hmcerror");
        }
        if (obschannel==2 && lee_strength_type2>100) {
            TFile* leefile = new TFile(Form("nueCC_PC_leefile_LEEx%d.root", (int)lee_strength_type2-100));
            hmclee = (TH1F*)leefile->Get("hmcerror");
        }
        /* if (obschannel==1 && lee_strength_type2>100) { */
        /*     TFile* leefile = new TFile(Form("nueCC_FC_0p_leefile_LEEx%d.root", (int)lee_strength_type2-100)); */
        /*     hmclee = (TH1F*)leefile->Get("hmcerror"); */
        /* } */
        /* if (obschannel==2 && lee_strength_type2>100) { */
        /*     TFile* leefile = new TFile(Form("nueCC_PC_0p_leefile_LEEx%d.root", (int)lee_strength_type2-100)); */
        /*     hmclee = (TH1F*)leefile->Get("hmcerror"); */
        /* } */
        /* if (obschannel==3 && lee_strength_type2>100) { */
        /*     TFile* leefile = new TFile(Form("nueCC_FC_Np_leefile_LEEx%d.root", (int)lee_strength_type2-100)); */
        /*     hmclee = (TH1F*)leefile->Get("hmcerror"); */
        /* } */
        /* if (obschannel==4 && lee_strength_type2>100) { */
        /*     TFile* leefile = new TFile(Form("nueCC_PC_Np_leefile_LEEx%d.root", (int)lee_strength_type2-100)); */
        /*     hmclee = (TH1F*)leefile->Get("hmcerror"); */
        /* } */
        ...
        legend[obschannel-1]->AddEntry(hCCpi0inFV, Form("#nu_{#mu} CC #pi^{0} in FV, %.1f", hCCpi0inFV->Integral()), "F"); 
        ...
        if(flag_leeexist && lee_strength_type2<100){
        ...
        if( (obschannel>=1 && obschannel<=2) && lee_strength_type2>100){
        legend[obschannel-1]->AddEntry(hmclee, Form("LEE(x=1), %.1f", hmclee->Integral()-hmc->Integral()), "l");
        hmclee->SetLineStyle(7);
        hmclee->SetLineWidth(4);
        hmclee->SetLineColor(kRed);
        }
        ...
        hmc->Scale(scalePOT);
        hmc->Draw("hist");
        hmc->GetYaxis()->SetTitle("Event counts / 100 MeV");
        hmc->GetYaxis()->SetTitleSize(0.07);
        hmc->GetYaxis()->SetTitleFont(132);
        hmc->GetYaxis()->SetTitleOffset(0.90);
        ...
        hmc->GetYaxis()->SetRangeUser(-0.05*mcymax, 2.2*mcymax);
        if(obschannel>=7 && obschannel<=8) { 
        hmc->GetYaxis()->SetRangeUser(-0.05*mcymax, 1.9*mcymax);
        }
        if(obschannel>=5 && obschannel<=6) { 
        hmc->GetYaxis()->SetRangeUser(-0.05*mcymax, 1.8*mcymax);
        }
        hmc->SetLineColor(kBlack);
        hmc->SetLineWidth(5);

        if( (obschannel>=1 && obschannel<=2) && lee_strength_type2>100) {
            hmclee->Draw("hist same");
        }
        ...
        // save out for other tests
        /* if(obschannel==1){ */
        /* TFile *cccvvv_input = new TFile("cv_input.root", "RECREATE"); */
        /* cccvvv_input->cd(); */
        /* gr[obschannel-1]->Write(); */
        /* hbadmatch->Write(); */
        /* hnumuCCinFV->Write(); */
        /* hnueCCinFV->Write(); */
        /* hNCinFV->Write(); */
        ...
        std::cout<<obschannel<<" "<<sumtotalcov[obschannel]<<" "<<hmc->Integral();
        double data_pred_ratio = hdata->Integral()/normalization/hmc->Integral();
        //hack: remove some bins
        if(obschannel==991 || obschannel==992){
        //if(obschannel==1 || obschannel==2 || obschannel==3 || obschannel==4){
        //if(obschannel==1 || obschannel==2 || obschannel==3 || obschannel==4 || obschannel==12 || obschannel==13 || obschannel==14 || obschannel==15){
         relerr_data = 1./TMath::Sqrt(hdata->Integral(7, 25));
         relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(7, 25);
         data_pred_ratio = hdata->Integral(7, 25)/normalization/hmc->Integral(7, 25);
        }
        /* if(obschannel==5 || obschannel==7 || obschannel==9){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(1, 9)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(1, 9); */
        /*  data_pred_ratio = hdata->Integral(1, 9)/normalization/hmc->Integral(1, 9); */
        /* } */
        /* if(obschannel==2 || obschannel==3){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(2, 7)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(2, 7); */
        /*  data_pred_ratio = hdata->Integral(2, 7)/normalization/hmc->Integral(2, 7); */
        /* } */
        /* if(obschannel==4 || obschannel==6){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(2, 8)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(2, 8); */
        /*  data_pred_ratio = hdata->Integral(2, 8)/normalization/hmc->Integral(2, 8); */
        /* } */
        /* if(obschannel==7){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(2, 9)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(2, 9); */
        /*  data_pred_ratio = hdata->Integral(2, 9)/normalization/hmc->Integral(2, 9); */
        /* } */
        /* if(obschannel==1){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(2, 7)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(2, 7); */
        ...
        /*  data_pred_ratio = hdata->Integral(2, 8)/normalization/hmc->Integral(2, 8); */
        /* } */
        /* if(obschannel==3 || obschannel==6){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(3, 8)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(3, 8); */
        /*  data_pred_ratio = hdata->Integral(3, 8)/normalization/hmc->Integral(3, 8); */
        /* } */
        /* if(obschannel==4){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(2, 9)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(2, 9); */
        ...
        /* } */
        /* if(obschannel==5){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(1, 5)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(1, 5); */
        /*  data_pred_ratio = hdata->Integral(1, 5)/normalization/hmc->Integral(1, 5); */
        /* } */
        /* if(obschannel==7){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(1, 10)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(1, 10); */
        /*  data_pred_ratio = hdata->Integral(1, 10)/normalization/hmc->Integral(1, 10); */
        /* } */
        /* if(obschannel==9){ */
        /*  relerr_data = 1./TMath::Sqrt(hdata->Integral(7, 15)); */
        /*  relerr_pred = TMath::Sqrt(sumtotalcov[obschannel])/hmc->Integral(7, 15); */
        /*  data_pred_ratio = hdata->Integral(7, 15)/normalization/hmc->Integral(7, 15); */
        /* } */
...
/* supplementary material: channel, Bin, Energy, eLEE, Intrinsic signal, Background, data 
        std::cout<<std::endl;
        std::cout<<"=============================="<<std::endl;
        if(obschannel==1)
        {
            ...
            std::cout<<"nueCC FC     \t"<<setw(2)<<i<<"     "<<(i-1)*0.1<<"-"<<i*0.1<<"     "
                <<hLEE->Integral(i,i)<<" "
                <<setw(8)<<hnueCCinFV->Integral(i,i)<<" "
                <<setw(8)<<hmc->Integral(i,i)-hLEE->Integral(i,i)-hnueCCinFV->Integral(i,i)<<" "
                <<setprecision(0)<<setw(8)<<hdata->Integral(i, i)<<"\n";
            }
        }
        if(obschannel==2)
        {
            ...
            std::cout.setf(ios::fixed, ios::floatfield);
            std::cout.precision(2);
            std::cout<<"nueCC PC     \t"<<setw(2)<<i<<"     "<<(i-1)*0.1<<"-"<<i*0.1<<"     "
                <<hLEE->Integral(i,i)<<" "
                <<setw(8)<<hnueCCinFV->Integral(i,i)<<" "
                <<setw(8)<<hmc->Integral(i,i)-hLEE->Integral(i,i)-hnueCCinFV->Integral(i,i)<<" "
                <<setprecision(0)<<setw(8)<<hdata->Integral(i, i)<<"\n";
        ...
        if(obschannel==3)
        {
            ...
            std::cout.setf(ios::fixed, ios::floatfield);
            std::cout.precision(2);
            std::cout<<"numuCC FC    \t"<<setw(2)<<i<<"     "<<(i-1)*0.1<<"-"<<i*0.1<<"     "
                <<hLEE->Integral(i,i)<<" "
                <<setw(8)<<hnumuCCinFV->Integral(i,i)<<" "
                <<setw(8)<<hmc->Integral(i,i)-hLEE->Integral(i,i)-hnumuCCinFV->Integral(i,i)<<" "
                <<setprecision(0)<<setw(8)<<hdata->Integral(i, i)<<"\n";
            }
        }
        if(obschannel==4)
        {
            ...
            std::cout.setf(ios::fixed, ios::floatfield);
            std::cout.precision(2);
            std::cout<<"numuCC PC    \t"<<setw(2)<<i<<"     "<<(i-1)*0.1<<"-"<<i*0.1<<"     "
                <<hLEE->Integral(i,i)<<" "
                <<setw(8)<<hnumuCCinFV->Integral(i,i)<<" "
                <<setw(8)<<hmc->Integral(i,i)-hLEE->Integral(i,i)-hnumuCCinFV->Integral(i,i)<<" "
                <<setprecision(0)<<setw(8)<<hdata->Integral(i, i)<<"\n";
        ...
        if(obschannel==5)
        {
            ...
            std::cout.setf(ios::fixed, ios::floatfield);
            std::cout.precision(2);
            std::cout<<"numuCCpi0 FC \t"<<setw(2)<<i<<"     "<<(i-1)*0.1<<"-"<<i*0.1<<"     "
                <<hLEE->Integral(i,i)<<" "
                <<setw(8)<<hCCpi0inFV->Integral(i,i)<<" "
                <<setw(8)<<hmc->Integral(i,i)-hLEE->Integral(i,i)-hCCpi0inFV->Integral(i,i)<<" "
                <<setprecision(0)<<setw(8)<<hdata->Integral(i, i)<<"\n";
            }
        }
        if(obschannel==6)
        {
            ...
            std::cout.setf(ios::fixed, ios::floatfield);
            std::cout.precision(2);
            std::cout<<"numuCCpi0 PC \t"<<setw(2)<<i<<"     "<<(i-1)*0.1<<"-"<<i*0.1<<"     "
                <<hLEE->Integral(i,i)<<" "
                <<setw(8)<<hCCpi0inFV->Integral(i,i)<<" "
                <<setw(8)<<hmc->Integral(i,i)-hLEE->Integral(i,i)-hCCpi0inFV->Integral(i,i)<<" "
                <<setprecision(0)<<setw(8)<<hdata->Integral(i, i)<<"\n";
        ...
        if(obschannel==7)
        {
            ...
            std::cout.setf(ios::fixed, ios::floatfield);
            std::cout.precision(2);
            std::cout<<"NCpi0        \t"<<setw(2)<<i<<"     "<<(i-1)*0.1<<"-"<<i*0.1<<"     "
                <<hLEE->Integral(i,i)<<" "
                <<setw(8)<<hNCpi0inFV->Integral(i,i)<<" "
                <<setw(8)<<hmc->Integral(i,i)-hLEE->Integral(i,i)-hNCpi0inFV->Integral(i,i)<<" "
                <<setprecision(0)<<setw(8)<<hdata->Integral(i, i)<<"\n";
            }
        }
        std::cout<<"=============================="<<std::endl;
*/
        ...
        gratio_mc[obschannel-1]->GetYaxis()->SetNdivisions(205);
        ...
        gratio_mc[obschannel-1]->GetYaxis()->SetTitleOffset(0.45);
        gratio_mc[obschannel-1]->GetYaxis()->SetTitleSize(0.14);
        gratio_mc[obschannel-1]->GetXaxis()->SetTitleOffset(0.9);
        gratio_mc[obschannel-1]->GetXaxis()->SetTitleSize(0.14);
        gratio_mc[obschannel-1]->GetYaxis()->SetLabelSize(0.12);
        ...
        if(obschannel>=-1 && obschannel<=-1) //hard coded at this moment
        {
            gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Reconstructed #pi^{0} kinetic energy (MeV)");
        }
        /* if(obschannel>=5 && obschannel<=7) //hard coded at this moment */
        /* { */
        /*     //gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Electron shower theta [Rad]"); */
        /*     //gratio_mc[obschannel-1]->GetXaxis()->SetTitle("E_{vis}^{rec} [MeV]"); */
        /*     gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Reco #pi^{0} energy [MeV]"); */
        /* } */
        //else gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Muon track theta [Rad]");
        //else gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Reco muon #phi (X-Y plane) [degree]");
        else gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Reconstructed E_{#nu} (MeV)");
        //gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Reco neutrino energy [MeV]");
        //if(obschannel>=8) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("nue BDT score");
        //if(obschannel>=8) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Reco neutrino energy [MeV]");
        //if(obschannel>=8) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Median dEdx (1-5 cm) [MeV/cm]");
        //if(obschannel==3) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("nue BDT score");
        ...
        /* if(obschannel==10) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("nue BDT score"); */
        /* if(obschannel==11) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("nue BDT score"); */
        /* if(obschannel==12) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Median dEdx (1-5 cm) [MeV/cm]"); */
        /* if(obschannel==4) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("vis_1_max_angle"); */
        /* if(obschannel==5) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("br3_1_iso_angle"); */
        /* if(obschannel==6) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("vis_1_max_angle"); */
        /* if(obschannel==7) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("br3_1_iso_angle"); */
        /* if(obschannel==8) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("vis_1_max_angle"); */
        /* if(obschannel==9) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("br3_1_iso_angle"); */
        ...
        /* if(obschannel==2) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower energy [MeV]"); */
        /* if(obschannel==3) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower cos#theta"); */
        /* if(obschannel==4) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower cos#theta"); */
        /* if(obschannel==5) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower #phi [degree]"); */
        /* if(obschannel==6) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower #phi [degree]"); */
        /* if(obschannel==7) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Ehadron [MeV]"); */
        /* if(obschannel==8) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Ehadron [MeV]"); */
        /*  if(obschannel==9) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Neutrino vtx X [cm]"); */
        /*  if(obschannel==10) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Neutrino vtx X [cm]"); */
        /*  if(obschannel==11) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Neutrino vtx Y [cm]"); */
        /*  if(obschannel==12) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Neutrino vtx Y [cm]"); */
        /*  if(obschannel==13) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Neutrino vtx Z [cm]"); */
        /*  if(obschannel==14) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Neutrino vtx Z [cm]"); */
        /* if(obschannel==9) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower energy [MeV]"); */
        /* if(obschannel==10) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower energy [MeV]"); */
        /* if(obschannel==11) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower cos#theta"); */
        /* if(obschannel==12) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower cos#theta"); */
        /* if(obschannel==13) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower #phi [degree]"); */
        /* if(obschannel==14) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower #phi [degree]"); */
        /* if(obschannel==15) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("nue BDT score"); */
        /* if(obschannel==3) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower energy [MeV]"); */
        /* if(obschannel==4) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower energy [MeV]"); */
        /* if(obschannel==5) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower cos#theta"); */
        /* if(obschannel==6) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower cos#theta"); */
        /* if(obschannel==7) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower #phi [degree]"); */
        /* if(obschannel==8) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower #phi [degree]"); */
        /* if(obschannel==9) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("nue BDT score"); */
        /* if(obschannel==5) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower energy [MeV]"); */
        /* if(obschannel==6) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower energy [MeV]"); */
        /* if(obschannel==7) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower cos#theta"); */
        /* if(obschannel==8) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower cos#theta"); */
        /* if(obschannel==9) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower #phi [degree]"); */
        /* if(obschannel==10) gratio_mc[obschannel-1]->GetXaxis()->SetTitle("Shower #phi [degree]"); */
        
        //hack end

        gratio_mc[obschannel-1]->GetXaxis()->SetTitleSize(0.14);
        gratio_mc[obschannel-1]->GetXaxis()->SetTitleOffset(0.90);
        gratio_mc[obschannel-1]->GetXaxis()->SetLabelSize(0.12);
        gratio_mc[obschannel-1]->GetYaxis()->SetTitleSize(0.14);
        gratio_mc[obschannel-1]->GetYaxis()->SetTitleOffset(0.45);
        gratio_mc[obschannel-1]->GetYaxis()->SetLabelSize(0.12);
        ...
        hist->GetYaxis()->SetRangeUser(0,int(1.5*maxratio)<3?int(1.5*maxratio):3);
        ...
        //if(flag_err==3) legend2[obschannel-1]->AddEntry(gratio_mc2[obschannel-1],"Pred xsec+flux uncertainty", "F");
