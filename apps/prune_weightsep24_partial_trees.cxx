// how to compile:
// g++ prune_weightsep24_partial_trees.cxx -o prune_weightsep24_partial_trees.exe `root-config --cflags --glibs`
//
// 04.15.2021 updated by Wenqiang Gu
//            save entries with partial knobs from GENIE
//
#include <cstdlib>
#include <iostream>
#include <fstream>
...
{
  if (argc < 3) {
    std::cout << "Usage: prune_weightsep24_partial_trees #txtInputList #outputROOT #GenieFluxKnob" << std::endl;
    ...
    if (mcweight_filled>0) {
      ...
      if (SaveGenieFluxKnob=="UBGenieFluxSmallUni") {
        ...
        for(auto const& knob: UBGenieFluxSmallUni) {
          ...
          if ( mcweight->find(knob) == mcweight->end() ) {
             // not found
             weight_f1.at(knob) = {};
          } else {
             // found
          ...
          if ( mcweight->find(SaveGenieFluxKnob) == mcweight->end() ) {
            knob_saved = false;
          } else {
            knob_saved = true;
